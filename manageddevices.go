...
func (s *Service) AddDevice(device models.Device) (id string, err error) {
	...
	common.LogCli.Debug(fmt.Sprintf("Adding Device: %v", device))

	id, err = common.DevCli.Add(&device)
	if err != nil {
...
func (s *Service) RemoveDevice(id string) error {
	...
	common.LogCli.Debug(fmt.Sprintf("Removing managed Device: : %v\n", device))
	err := common.DevCli.Delete(id)
	if err != nil {
		common.LogCli.Error(fmt.Sprintf("Delete Device %s from Core Metadata failed", id))
		return err
	}
...
func (s *Service) RemoveDeviceByName(name string) error {
	...
	common.LogCli.Debug(fmt.Sprintf("Removing managed Device: : %v\n", device))
	err := common.DevCli.DeleteByName(name)
	if err != nil {
		common.LogCli.Error(fmt.Sprintf("Delete Device %s from Core Metadata failed", name))
		return err
	}
...
func (s *Service) UpdateDevice(device models.Device) error {
	...
	common.LogCli.Debug(fmt.Sprintf("Updating managed Device: : %v\n", device))
	err := common.DevCli.Update(device)
	if err != nil {
		common.LogCli.Error(fmt.Sprintf("Update Device %s from Core Metadata failed: %v", device.Name, err))
		return err
	}
